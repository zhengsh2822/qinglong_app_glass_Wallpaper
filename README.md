...
> 本项目在原项目基础上进行了主题系统重构、多账号安全增强、仪表盘功能扩展、脚本搜索能力补全、京东助手独立模块化、全局可更换壁纸系统、统一毛玻璃组件体系等改进。
## 相关项目
...
- 卡片新设计：左侧状态竖条、状态胶囊标签（运行中/待机/已禁用）、一键运行/停止按钮
- 任务详情、即时日志、历史日志查看
...
- 账号与青龙配置备份/恢复
- 添加/编辑/删除账号弹窗统一使用模糊展开动画与卡片样式

### 其他功能

- 配置文件管理、依赖管理（缺失依赖扫描）、登录日志、任务日志
- 应用内购买、推送设置、字体设置（字体大小 + 字体粗细四档 + 主/次字体颜色自定义 HSL 选色器）、修改密码、账号排序
- 检查更新、关于页面、依赖设置、备份恢复、模糊度与卡片纯色调节
- 全局可更换壁纸系统（渐变/纯色/本地图片/网络图片），支持模糊度、暗化与卡片纯色调节
- 悬浮时钟：SNTP 多源独立时间校准（阿里云/腾讯云/国家授时中心等 5 源取时），不跟随系统时间，支持 iOS 风格文本选择放大镜
- 底部液态玻璃导航栏 + 顶部毛玻璃分段 Tab，适配安卓小白条和状态栏沉浸效果；双击导航栏/状态栏回页面顶部
