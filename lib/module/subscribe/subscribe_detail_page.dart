...
class SubscribeDetailPage extends ConsumerStatefulWidget {
  ...
  const SubscribeDetailPage(this.taskBean, {Key? key, this.hideAppbar = false})
    : super(key: key);
...
class _TaskDetailPageState extends ConsumerState<SubscribeDetailPage>
    with LazyLoadState<SubscribeDetailPage> {
  ...
  void _enableSubscribe(BuildContext context, int disabled) {
    ref
        ...
        .enableSubscribe(context, widget.taskBean["id"], disabled);
  }
          ...
          CupertinoButton(
            ...
            onPressed: () {
              showMoreOperate(context, [
                CupertinoSheer(
                  ...
                  onTap: () async {
                    if (widget.taskBean["status"] == 1) {
                      await _startCron(context, ref, true);
                    } else {
                ...
                CupertinoSheer(
                  title:
                      (widget.taskBean["is_disabled"] ?? 0) == 1 ? "禁用" : "启用",
                  onTap: () {
                    _enableSubscribe(
                      context,
                      (widget.taskBean["is_disabled"] ?? 0),
                    );
                  },
                ),
              ]);
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SubscribeDetailCell(
                        title: "订阅状态",
                        desc:
                            (widget.taskBean["is_disabled"] ?? 0) == 1
                                ? "已禁用"
                                : "已启用",
                      ),
                      SubscribeDetailCell(
              ...
              CupertinoDialogAction(
                ...
                onPressed: () async {
                  ...
                  await ref
                      ...
                      .delSubscribe(context1, widget.taskBean["id"]);
  ...
  void showLog() {
    Navigator.of(context).push(
      WallpaperPageRoute(
        builder:
            (context) => InTimeSubscribeLogPage(
              widget.taskBean["id"],
              true,
  ...
  _startCron(BuildContext context, WidgetRef ref, bool log) async {
    await ref
        ...
        .runCrons(context, widget.taskBean["id"]);
    if (log) {
  ...
  _stopCron(BuildContext context, WidgetRef ref) {
    ref
        ...
        .stopCrons(context, widget.taskBean["id"]);
  }
  ...
  Widget _buildOpenPub() {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SubscribeDetailCell(title: '分支', desc: widget.taskBean >> "branch"),
        ],
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          ...
          Visibility(
            ...
            child: const SubscribeDetailCell(title: '私钥', desc: ""),
          ),
          Visibility(
            visible: pullType == PullType.userName,
            child: const SubscribeDetailCell(title: '用户名', desc: ""),
          ),
          Visibility(
            visible: pullType == PullType.userName,
          ...
          Visibility(
            visible: pullType == PullType.userName,
            child: const SubscribeDetailCell(title: "密码/Token", desc: ""),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Column(
      ...
      children: [
        ...
        hideDivide ? const SizedBox.shrink() : const Divider(height: 1),
      ],
    );
  }
}
