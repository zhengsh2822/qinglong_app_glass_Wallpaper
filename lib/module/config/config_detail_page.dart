...
class ConfigDetailPage extends ConsumerStatefulWidget {
  ...
  const ConfigDetailPage({Key? key, required this.bean, this.content})
    : super(key: key);
...
class _ConfigDetailPageState extends ConsumerState<ConfigDetailPage>
    with LazyLoadState<ConfigDetailPage> {
            ...
            onPressed: () async {
              ...
              showMoreOperate(context, [
                ...
                CupertinoSheer(
                  title: "分享",
                  onTap: () {
                    ShareUtils.share(sourceContent ?? "");
                  },
                ),
                addDivider(),
                CupertinoSheer(
                  ...
                  onTap: () async {
                    try {
                      String name =
                          "${Random().nextInt(40000)}_${widget.bean.title}";
                      var file = await FileUtil(
                        0,
                      ).writeDownloadFile(name, sourceContent ?? "");
          ...
          isLoading
              ...
              : SafeArea(
                ...
                child: SelectableCodeView(
                  ...
                  language: LanguageUtils.fromFileName(widget.bean.title ?? ""),
                ),
              ),
    );
  }
