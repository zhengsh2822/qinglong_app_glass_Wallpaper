...
class _PushSettingPageState extends ConsumerState<PushSettingPage> {
  ...
  void _showPushSelector(BuildContext context) {
    // 与主题版一致：复用通用底部弹窗选择器（lib/base/ui/selector_sheet.dart），
    // 选项带副标题（"已关闭通知"/"N 项配置"），避免自定义弹窗重复实现
    final options = list
        .map(
  ...
  @override
  void dispose() {
    for (var push in list) {
      for (var child in push.children) {
  ...
  void commit() async {
    if (current == null || current!.key == "已关闭") {
      ...
      var response = await SingleAccountPageState.ofApi(
        context,
      ).updateNotifcation({});
      EasyLoading.dismiss();
      if (response.success) {
    ...
    int? index = current?.children.indexWhere(
      (element) => element.controller.text.isEmpty && element.required == true,
    );
    index ??= -1;
    if (index >= 0) {
