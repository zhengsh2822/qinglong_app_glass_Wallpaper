...
class LoginHelper {
  ...
  Future<int> login(BuildContext context) async {
    ...
    if (loginByUserName()) {
      response = await SingleAccountPageState.ofApi(
        context,
    ...
    if (response.success) {
      ...
      //可能用户使用的是老版本qinglong
      HttpResponse<LoginBean> oldResponse = await SingleAccountPageState.ofApi(
        context,
  ...
  Future<int> loginTwice(BuildContext context, String code) async {
    HttpResponse<LoginBean> response = await SingleAccountPageState.ofApi(
      context,
